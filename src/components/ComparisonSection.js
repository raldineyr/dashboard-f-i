...
export class ComparisonSection {
  ...
  render(container) {
    ...
    if (!container) {
      console.error(
        'ComparisonSection: container não encontrado'
      );
  ...
  // =========================================================
  // CÁLCULO DE VARIAÇÃO ENTRE DOIS VALORES
  // 
  // REGRA: compara o valorBase com o valorComparado
  // - Se valorComparado > valorBase → positivo
  // - Se valorComparado < valorBase → negativo
  // - Se iguais → 0,0%
  // =========================================================
  ...
  calculateVariationBetween(valueBase, valueComparado) {
    ...
    // Base é zero e comparado é maior que zero
    if (base === 0 && comparado > 0) {
      return {
        text: '+100,0%',
    ...
    // Base é zero e comparado é menor que zero
    if (base === 0 && comparado < 0) {
      return {
        text: '-100,0%',
    ...
    // Cálculo normal: (comparado - base) / |base| * 100
  ...
  renderComparison() {
    ...
    this.renderAutomaticComparison(
      content
    );
  }
  ...
  renderAutomaticComparison(
    ...
    const stores =
      this.getComparisonStores();


    const validStores =
      stores.filter(
        store =>
          store.months.length >= 2
      );
    ...
    if (!selected) {
      ...
      content.innerHTML = `
        ...
        <div class="comparison-empty">
          ...
          Selecione uma loja com pelo menos
          dois meses carregados.
    ...
    if (stores.length < 2) {
      ...
      content.innerHTML = `
        <div class="comparison-empty">
          ...
          É necessário carregar pelo menos
          duas lojas para realizar este comparativo.
  ...
  renderSellerComparison(
    content
  ) {

    const sellers =
      this.getComparisonSellers();
    ...
    const rows =
      indicators.map(
        (indicator) => {
          ...
          // =========================================================
          // VARIAÇÃO DO VENDEDOR A EM RELAÇÃO AO VENDEDOR B
          // =========================================================
          // Se A > B → positivo
          // Se A < B → negativo
          // Se A = B → 0
    ...
    const rows =
      indicators.map(
        indicator => {
          ...
          // =========================================================
          // VARIAÇÃO DA LOJA A EM RELAÇÃO À LOJA B
          // =========================================================
          // Se A > B → positivo
          // Se A < B → negativo
          // Se A = B → 0
  ...
  // =========================================================
  // SELECT — REGISTROS DOS VENDEDORES
  // =========================================================

  buildSellerRecordOptions(
    records,
    selected
  ) {
