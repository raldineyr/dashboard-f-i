...
export class ComparisonSection {
  ...
  getStoreIdentityText(data) {
    ...
    // A identidade da seção deve ter prioridade sobre o nome do arquivo.
    // Um único arquivo pode conter VDC + FSA, portanto usar o filename
    // como parte principal da identidade pode misturar duas lojas.
    return normalizeKey(
      [
        data.name,
        data.label,
  ...
  getCanonicalStore(data) {
    if (!data) {
      return { key: '', label: 'Loja' };
    }

    // Se o DataExtractor já forneceu uma storeKey válida, ela é a fonte
    // mais confiável para o comparador.
  ...
  getPeriodKey(data) {
    ...
    if (!month && data.monthLabel) {
      const normalizedMonth = normalizeKey(data.monthLabel);
      const monthNames = {
        JANEIRO: 1,
        FEVEREIRO: 2,
        MARCO: 3,
      ...
      month = monthNames[normalizedMonth] || 0;
    }
  ...
  extractYearFromSource(sourceFile) {
    ...
    // Suporta nomes de arquivo como JULHO-26.xlsx.
    const shortYear = value.match(/(?:^|[-_\s])([0-9]{2})(?=$|[-_\s.]|\b)/);
    if (shortYear) {
